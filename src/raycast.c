...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/19 13:28:31 by ppepperm          #+#    #+#             */
/*   Updated: 2020/05/19 13:28:32 by ppepperm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				raycast(t_data data, t_player player, SDL_Renderer *renderer)
{
	...
	data.param.x = 0;
	if (!put_skybox(renderer, data, player))
		return (0);
	while (data.param.x <= W_W)
	{
		...
		while (data.param.ray_status == GOING)
		{
			...
			if (texture_selection(&data, renderer, &ray, player) < 0)
				return (0);
		}
		data.param.x++;
	}
